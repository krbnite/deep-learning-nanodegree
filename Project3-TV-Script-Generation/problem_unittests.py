...
def test_get_batches(get_batches):
    with tf.Graph().as_default():
        ...
        # shape[2] corresponds to batch_size:  x is seq_num of the seq in given batch
        # We are looking at the 0th batch, the 0th data type (input), all rows (all 128 seqs of length 5)
        #     0th row: [0,5)
        #     1st row: [35,40)
        #     2nd row: [70,75) ...
        # The 35 comes from dim0*dim3 = 7*5 = num_batches*seq_length
        ...
        # We are looking at the 0th batch, the 1st data type (targets), all rows (all 128 seqs of length 5)
